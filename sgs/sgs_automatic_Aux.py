...
ti_gan_time = 5.95  # 提杆后，过多少秒就是开钓鱼 这个最好是以加大时间调整,最低时间为5，在往下调,会出bug
...
# 上
i_up = r'E:\sgs_auto\sgs\sgs_image\01_up.png'
# 左
i_left = r'E:\sgs_auto\sgs\sgs_image\02_left.png'
# 下
i_un = r'E:\sgs_auto\sgs\sgs_image\03_un.png'
# 右
i_right = r'E:\sgs_auto\sgs\sgs_image\04_right.png'
# 风
i_wind = r'E:\sgs_auto\sgs\sgs_image\05_wind.png'
# 火
i_fire = r'E:\sgs_auto\sgs\sgs_image\06_fire.png'
# 雷
i_ray = r'E:\sgs_auto\sgs\sgs_image\07_ray.png'
# 电
i_electricity = r'E:\sgs_auto\sgs\sgs_image\08_electricity.png'

def parser():
    parser = argparse.ArgumentParser(description="Nothing")
    parser.add_argument('--path_of_image',type=str, default="E:\\sgs_auto\\sgs\\ ",
                        help="文件的根目录")
    parser.add_argument('-n','--loop_num',type=int, help="为你本次某个鱼饵的数量，填写几个就循环钓鱼几次")
    parser.add_argument('-s','--sleep_duration', type=int, default=8,
                        help="每轮结束后的等待时间")
...
def go_fish(args):
    ...
    for epoch in range(args.loop_num):
        global last_gan_time
        global initial_color
        last_gan_time = 0
        # Step 5: 抛竿
        initial_color = pyautogui.pixel(window_x +600, window_y +1000)
        ...
        son_threa_run = True
        print(f"第{epoch+1}次，总数为{args.loop_num}")
        while True:
            with gan_lock:
...
def press_keys():
    ...
    pyautogui.press('2')
    print("按下了1和2")
    # threading.Timer(3,press_keys()).start()

def check_color_change():
    """检查颜色是否变化，并重新设置定时器"""
    global initial_color
    current_color = pyautogui.pixel(window_x+600, window_y+1000)
...
def all_flow(args):
    game_window = gw.getWindowsWithTitle('MuMu模拟器12')[0]  # 替换为实际的游戏窗口标题
    # 获取窗口位置
    global window_x
